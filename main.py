...
def handle_packet(data: bytes):
    if len(data) >= PACKET_HEADER_FORMAT_SIZE:
            ...
            remaining_bytes = data[PACKET_HEADER_FORMAT_SIZE:]
            ...
            match packet_header.packet_id:
                ...
                case PacketIDs.CAR_DAMAGE.value:
                    car_damage_packet = unpack_car_damage(packet_header, remaining_bytes)
                ...
                case PacketIDs.CAR_STATUS.value:
                    car_status_packet = unpack_car_status(packet_header, remaining_bytes)
                ...
                case PacketIDs.CAR_TELEMETRY.value:
                    car_telemetry_packet = unpack_car_telemetry(packet_header, remaining_bytes)
                ...
                case PacketIDs.EVENT.value:
                    event_packet = unpack_event_packet(packet_header, remaining_bytes)

                    print(f"Received and unpacked Event Packet for event of type {type(event_packet.event).__name__}")

                case PacketIDs.TYRE_SETS.value:
                    tyre_sets_packet = unpack_tyre_sets(packet_header, remaining_bytes)
